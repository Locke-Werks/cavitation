...
/// Which transport a conversation rides on.
///
/// Three tiers, not two: RCS is its own thing and reads differently from a
/// green-bubble SMS thread. The wire format distinguishes all three (the raw
/// SMS structs carry `mode` = sms/mms and `ic` = 2 for RCS), but rustpush
/// collapses RCS into `MessageType::SMS` before it reaches us. receive.rs is
/// where that gets sorted out once the core reports it.
pub const SERVICE_IMESSAGE: &str = "iMessage";
pub const SERVICE_RCS: &str = "RCS";
...
impl Storage {
    ...
    /// database made before `service` existed never gains the column from
    /// SCHEMA. Add it here and backfill from the old boolean.
    fn migrate(conn: &Connection) -> Result<()> {
        let has_service: bool = conn
            .prepare("SELECT 1 FROM pragma_table_info('chats') WHERE name = 'service'")?
            .exists([])?;

        if !has_service {
            conn.execute_batch(
                "ALTER TABLE chats ADD COLUMN service TEXT NOT NULL DEFAULT 'iMessage';",
            )?;
        }

        // Runs every open: cheap, and it repairs rows written by an older
        // build that set is_imessage without a service.
        conn.execute(
            "UPDATE chats SET service = CASE WHEN is_imessage = 1 THEN ?1 ELSE ?3 END
             WHERE service IS NULL OR service = '' OR service NOT IN (?1, ?2, ?3)",
            params![SERVICE_IMESSAGE, SERVICE_RCS, SERVICE_SMS],
        )?;
        Ok(())
    }
    ...
    pub fn upsert_chat(
        ...
        // is_imessage is kept in step with service so an older build reading
        // this database still sorts blue from green.
        let is_imessage = service == SERVICE_IMESSAGE;
        self.conn.execute(
            "INSERT INTO chats(guid, title, is_imessage, service, is_group, created_at)
             VALUES(?,?,?,?,?,?)
             ON CONFLICT(guid) DO UPDATE SET title=excluded.title,
             is_imessage=excluded.is_imessage, service=excluded.service,
             is_group=excluded.is_group",
            params![guid, title, is_imessage as i64, service, is_group as i64, now_epoch()],
        )?;
        Ok(())
    }
    ...
    pub fn list_chats(&self, include_archived: bool) -> Result<Vec<ChatRow>> {
        let sql = if include_archived {
            "SELECT guid, title, service, is_group, is_archived, is_pinned, pin_order,
    ...
    /// Raw addresses for a chat, which is what IDS wants as participants.
    /// participants_summary is for display and resolves display names, so it
    /// cannot be reused here.
    pub fn participant_addresses(&self, chat_guid: &str) -> Result<Vec<String>> {
        let mut stmt = self.conn.prepare(
            "SELECT h.address FROM chat_handles ch
             JOIN handles h ON h.id = ch.handle_id
             WHERE ch.chat_guid = ? ORDER BY h.address",
        )?;
        let rows = stmt
    ...
    pub fn messages_for_chat(&self, chat_guid: &str, limit: i64) -> Result<Vec<MessageRow>> {
        let mut stmt = self.conn.prepare(
            // LIMIT applies after ORDER BY, so ordering ascending here would
            // take the OLDEST rows and a 2000-message chat read at limit 500
            // would show messages 1-500 with the newest 1500 invisible. Take
            // the newest N descending, then flip back to ascending for display.
            "SELECT * FROM (
             ...
             WHERE m.chat_guid = ? ORDER BY m.date DESC LIMIT ?
             ) ORDER BY date ASC",
        )?;
