...
fn account_path(data_dir: &Path) -> PathBuf { data_dir.join("account.json") }
...
pub async fn create_session(data_dir: &Path, config: Arc<dyn OSConfig>) -> Result<AuthSession> {
    ...
    // Upstream now hands back an account blob (tokens, adsid, dsid) and takes
    // the previous one, which is what lets a restart skip the 2FA round trip.
    // The callback fires whenever the tokens are refreshed, so the file has to
    // be rewritten then rather than only at login.
    let persisted: Option<PersistAccountData> =
        load_json(&account_path(data_dir)).unwrap_or(None);
    let persist_target = account_path(data_dir);

    let account = AppleAccount::new_with_anisette(
        login_config,
        anisette.clone(),
        persisted,
        Box::new(move |data: &PersistAccountData| {
            // A failure here costs the next launch its saved login, which is
            // an inconvenience, not a reason to take the session down.
            if let Err(e) = save_json(&persist_target, data) {
                log::warn!("persist account data: {e}");
            }
        }),
    )
