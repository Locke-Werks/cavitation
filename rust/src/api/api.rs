...
// configure_app_review() was removed here, along with id_testing.plist and
// hw_testing.plist.
//
// Upstream used it to drop a pre-registered iMessage identity so an App Store
// reviewer could skip sign-in. The two plists it embedded held live IDS and
// APNs private keys, the registered handles for that account, and a real Mac's
// serial, platform UUID and MAC address. include_str! meant every binary built
// from this tree carried them, whether or not anything called the function.
//
// There is no App Store review for a Windows build, so it was dead weight that
// shipped someone else's credentials. If an equivalent is ever needed, it
// should read from a file the user supplies at runtime, never from the binary.
